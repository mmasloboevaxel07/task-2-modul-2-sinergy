...
public class HomeWork2 {
    ...
    public static void validateLogin(String login) throws LoginValidationException {
        if(!(login.matches("[A-Za-z1-9_]+") & !login.matches("[a-z1-9_]+") & !login.matches("[A-Z1-9_]+") & login.length() <= 20 & login.contains("_"))) {
    ...
    public static Boolean isLoginValid(String login) {
        ...
        try {
            validateLogin(login);
        } catch (LoginValidationException loginValidationException) {
            isRight = false;
        }
        return isRight;
    }
